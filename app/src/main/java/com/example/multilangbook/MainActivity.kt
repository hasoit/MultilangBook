...
//        Text("$found")
    ...
    fun app() {
        ...
        var showDictionaryExplorer by remember { mutableStateOf(false) }
        var showMulticolorWords by remember { mutableStateOf(false) }
        ...
        var swipeTreshold by remember { mutableStateOf(8) }
        ...
        fun switchOptionButtons() = switchButtons(
            ...
            SwitchButton("Swipe", swipe) { swipe = !swipe },
        ...
        fun settings() {
            ...
            if (swipe) intSetting("Swipe treshold: $swipeTreshold", { swipeTreshold++ }, { swipeTreshold-- })
        ...
        fun sentenceNumber(i: Int) {
            ...
            Row {
                ...
                Text(
                    ...
                    fontSize = 24.sp//(2*textsize).sp // fontSize = 32.sp,
                )
        ...
        fun mainView() {
            ...
            Column(
                ...
                Column(
                    maxwidth().verticalScroll(scrollState).weight(1f).padtb(16).pointerInput(Unit) {
                        detectHorizontalDragGestures(
                            { swipeHandled = false }, { swipeHandled = true }, onHorizontalDrag = { _, dragAmount ->
                                if (!swipeHandled) {
                                    swipeHandled = true
                                    dbp(dragAmount)
                                    if (abs(dragAmount) > swipeTreshold)
                                        if (dragAmount < 0) {
                                            if (swipe) increaseIndex() else {
//                                                showmenu = false
//                                                showsettings = !showsettings
                                            }
                                        } else {
                                            if (swipe) decreaseIndex() else {
//                                                showsettings = false
//                                                showmenu = !showmenu
                                            }
                                        }
